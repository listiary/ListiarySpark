<?php
	...
	// Find all files with the .json extension in the directory
	$files = glob($sourceDir . "*.json");
	if (empty($files)) 
	{
		echo "No .json files found in '{$sourceDir}'." . NEW_LINE;
		exit(0); // Exit cleanly as there is nothing to do
	}
	echo "Found " . count($files) . " .json file(s). Starting validation and upload..." . NEW_LINE;
